...
compute_KL_divergence([X|ProposalDistribution], [Y|ObjectiveDistribution], KLDivergence) :-
	...
	KLDivergence is KLStepPrevious + ( X * log( (X+0.0000000000000000000000000001) / (Y+0.0000000000000000000000000001) ) ).
...
traverse_prior_adverbs_max(ProposalDistribution, Resolution, Max, Current, PriorId, ChosenPrior):-
	...
	normalize_distribution(PriorDistribution, N),
        ...
        traverse_prior_adverbs_max(ProposalDistribution, Resolution, Max, KLDivergenceProposed, NextPrior, ChosenPrior).
...
traverse_prior_adverbs_max(ProposalDistribution, Resolution, Max, Current, PriorId, _):-
	...
	normalize_distribution(PriorDistribution, N),
        ...
        traverse_prior_adverbs_max(ProposalDistribution, Resolution, Max, Current, NextPrior, PriorId).
...
dcompute_max_KL_divergence(ProposalDistribution, Resolution, Kl):-
        traverse_prior_adverbs_max(ProposalDistribution, Resolution, Kl, 0, 1, 1).

further_distrib([],[]).
further_distrib([L|_],_):-
	L > 0.999999999999999999999999999,
	halt.
further_distrib([L|_],_):-
        L < 0.0000000000000000000000000001,
        halt.
further_distrib([L|Lik],[0.0000000000000000000000000001|NoLik]):-
        M is max(abs(L-0.999999999999999999999999999),L),
        M == L,
        further_distrib(Lik,NoLik).

further_distrib([_|Lik],[0.999999999999999999999999999|NoLik]):-
        further_distrib(Lik,NoLik).

icompute_max_KL_divergence(Lik,_,MaxKL):-
        further_distrib(Lik,NoLik),
        compute_KL_divergence(Lik,NoLik,MaxKL).
...
prob_fake_new(_, LikelihoodDistribution, PosteriorDistribution, ProbFakeNew, Resolution) :-
	...
	DivergenceFactor is 1.0-exp(-Kl),
	EntropyFactor is 1.0 - (PosteriorEntropy / Maximum), 
	ProbFakeNew is (0.8*DivergenceFactor + 0.2*EntropyFactor)**3.
...
% Execution command: prob_fake_new([0.2,0.2,0.6],[0.1,0.2,0.7],[0.98,0.01,0.01],P,3).
...
main(Cause, Effect, PosteriorDistribution, Resolution, File, PosteriorAdverb, PriorEntropy, PosteriorEntropy, MAP, Argmax, FileR):-
