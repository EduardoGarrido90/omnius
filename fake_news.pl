:- include('priors.pl').
:- include('experiments/fake/synthetic/first_experiment/train.pl').
...
%Ejemplo llamada: prob_fake_new([0.2,0.2,0.6],[0.5,0.3,0.2],[0.98,0.01,0.01],P,3).
...
%Ejemplo llamada: compute_prob_fake_new([0.2,0.2,0.6],[0.1,0.2,0.7],[0.98,0.01,0.01],P,3).	
compute_prob_fake_new(PriorDistribution, LikelihoodDistribution, PosteriorDistribution, 1, Resolution) :-
	...
	P > 1,
	open("prob_fake", append, FilePointer),
	write(FilePointer,P),
	write(FilePointer,"\n"),
	close(FilePointer),
	!.
